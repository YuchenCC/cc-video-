...
- 中央主视区：只显示当前讲解节点；顶部保留约 22%（238px），底部保留约 22%（238px），有效区域为 `x=480–1440`、`y=238–842`。
- 底部字幕安全区：从 `y=842` 延伸至画布底部。主概念、历史节点和人物主体不得进入字幕条所在区域。
...
- 例外：当素材不是独立概念，而是“植入父概念的增强件、插件或能力徽章”时，可以使用白底圆角徽章承载，目的是让小图标在 StackFrame、终端或复杂线框上保持可读。徽章应保持中性白底、轻阴影、无彩色描边，飞入峰值通常不超过 `1.5` 倍。
- 当前概念在中央有效区域内尽量撑开。图片节点必须保持原始宽高比，并将可见高度限制在中央有效区的约 78%（参考高度 440px）；使用顶部对齐，为字幕保留明确的视觉间隔。

## 从第一幕反推的模板槽位

`scene-001` 验证后，`concept-expansion` 不再只表示“父子节点链”，而是一个可复用的白板叙事容器。未来场景优先从以下槽位中选择，避免为每个 Scene 临时重造结构：

| 槽位 | 用途 | 推荐位置 | 典型动画 |
|---|---|---|---|
| `definition-asset` | 单一概念定义，例如一个由多层图片组成的组件图标 | 中区主视图，尽量撑开 | 整体 `fade/pop in`，作为一体同时出现/消失 |
| `entry-stack` | 多个入口、层级或候选对象组成的清单 | 中区主视图 | StackFrame 先出现，StackLabel 按口播逐项进入 |
| `enhancer-badge` | 插件、接入件、补丁、能力增强 | 父概念边角、插槽或内部增强位 | 从右上/右侧飞入，峰值约 `1.5`，随后植入或替换为稳定徽章 |
| `state-replacement` | 旧状态变为新状态、风险变为覆盖结果 | 原行原位 | 旧 Label 缩小淡出，新 Label 原位进入，CheckBadge 主 timeline 绘制 |
| `context-character` | 人物情绪、人工负担或结果反馈 | 右区 | 与当前语义同步进入；不得遮挡中区或字幕 |
| `context-artifact` | 终端、依赖片段、记录、脚本等补充证据 | 左区或右区 | 作为上下文晚于主概念出现，不抢主焦点 |

这些槽位可以在同一个 Scene 中连续使用，但同一时刻只能有一个主焦点。右侧人物和左右辅助物料只解释语义背景，不承担新的主概念。
...
5. 增强件可以短暂显示为白底徽章；若最终语义是“已接入某技术能力”，可在植入后替换成稳定的技术徽章，继续留在父概念层内。
...
- 颜色切换必须表达状态变化，不随机轮换。
- Label 文案必须在 Label 中央对齐；固定宽度是为了稳定版面，不是让短文案贴边。
- 如果 StackFrame 是增强接入的父概念，Frame 在接入瞬间保持主屏不动；只允许轻微脉冲或边框语义变色，不做左移。
...
第一幕沉淀出一套适合技术讲解开场或复杂概念拆解的节奏：

1. **定义概念**：中区展示一个完整概念图标或图解，多个内部图片必须作为一个整体同时出现、同时消失。
2. **列出入口**：切到 StackFrame 清单，Frame 稳定居中，Label 按口播逐项进入。
3. **给出负担/问题背景**：右区人物或少量上下文物料出现；主焦点仍是中区清单。
4. **接入增强件**：增强件从右上飞入父概念，并植入父概念内部；父概念不左移。
5. **逐项覆盖**：每次只替换一个 Label，并绘制对应 CheckBadge；其它行可降到 `0.35` 后在阶段结尾恢复。
6. **结果反馈**：人物情绪或辅助物料变化作为结论反馈，主概念保持在画面中。

这种编排适合“原有对象 + 接入能力 + 逐项覆盖/改善”的讲解。若语义是严格父子层级，则回到“父概念左移、子概念居中”的标准扩展链。
...
- 中央是否始终只有一个完整高亮节点？
- 中央主视区是否占画面 50%，顶部和底部是否各保留约 22% 安全留白？
- 所有主要内容是否停留在 `y=238–842`，没有进入底部字幕安全区？
...
- 是否一次只扩展一个层级？
- 引入的图片图标是否保持无边框、无底板？
- 若图标使用白底，是否确实属于增强件/徽章，而不是普通素材节点？
- StackFrame 是否宽度稳定，所有 Label 是否同宽且居中？
- 增强型接入时父概念是否保持主屏幕不动？
- CheckBadge 是否由主 GSAP timeline 绘制，seek 后仍正确？
- 人物、终端、记录或脚本是否只是上下文，没有抢占中央主焦点？
