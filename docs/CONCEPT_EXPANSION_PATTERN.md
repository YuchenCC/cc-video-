...
- 中央主视区：只显示当前讲解节点；上下各保留约 20% 画布高度（216px），有效区域为 `x=480–1440`、`y=216–864`。
...
- 当前概念在中央有效区域内尽量撑开。图片节点必须保持原始宽高比并放大到不超过区域边界的最大尺寸，使用 `width: 100%; height: 100%; object-fit: contain`。
...
- 中央是否始终只有一个完整高亮节点？
- 中央主视区是否占画面 50%，上下是否各保留约 20% 安全留白？
